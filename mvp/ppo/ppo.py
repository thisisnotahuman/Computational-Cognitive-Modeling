...
class PPO:
    ...
    def log(
        ...
        wandb.log({'Mean reward': rewbuffer_mean})
        wandb.log({'Value function loss': mean_value_loss})
        wandb.log({'Surrogate loss': mean_surrogate_loss})
        wandb.log({'Mean episode length': lenbuffer_mean})
        wandb.log({'Mean success rate': mean_success_rate})
